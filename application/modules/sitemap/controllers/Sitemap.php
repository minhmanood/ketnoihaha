...
class Sitemap extends Layout {
	...
	function ajax_created() {
		...
		$message['message'] = show_alert_warning('Kiểm tra thông tin nhập');

		$post = $this->input->post();
		if (!empty($post)) {
			$this->form_validation->set_error_delimiters('<span class="help-block">', '</span>');
			$this->form_validation->set_rules('allowed_sitemap', '', 'required');
				...
				if ($this->input->post('allowed_sitemap')) {
					...
					if (is_array($shops) && !empty($shops)) {
						foreach ($shops as $value) {
							$lastmod = $value['modified'] > 0 ? $value['modified'] : $value['created'];
							$items[] = array(
					...
					$message['message'] = show_alert_success($action . ' sitemap thành công!');
				}
			}
		}
