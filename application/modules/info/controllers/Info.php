...
class Info extends Layout
{
    ...
    function __construct()
    {
        ...
        $this->_path = get_module_path('info');
    }
        ...
        if (!empty($post)) {
            ...
            if ($this->form_validation->run($this)) {
                if ($this->input->post('id')) {
                    ...
                    if ($err === FALSE) {
                        $this->_upload_images($id, 'attributes');
                        $notify_type = 'success';
                    ...
                    if ($err === FALSE) {
                        $this->_upload_images($insert_id, 'attributes');
                        $notify_type = 'success';
    ...
    function admin_content_1()
    {
        $post_type = $this->input->post('post_type');
        $attributes = $this->input->post('attributes');
        // $attributes['image'] = array();

        // // Lưu tên ảnh vào mảng $attributes
        ...
        // // Xóa mảng con image trong $_FILES
        // $number_of_files = sizeof($_FILES['attributes']['name']['image']);
        // foreach ($_FILES['attributes'] as $key => $value) {
        //     for ($i = 0; $i < $number_of_files; $i++) {
        //         $_FILES['attributes'][$key][$i] = $_FILES['attributes'][$key]['image'][$i];
        //     }
        //     unset($_FILES['attributes'][$key]['image']);
        // }

        // // Lấy các vị trí không up ảnh
        // $array_index_img_null = array();
        // foreach ($_FILES['attributes']['name'] as $key => $value) {
        //     if (trim($value) == '') {
        //         $array_index_img_null[] = $key;
        //     }
        // }

        // // Xóa các phần tử ảnh null
        // foreach ($_FILES['attributes'] as $key => $value) {
        //     foreach ($array_index_img_null as $index) {
        //         unset($_FILES['attributes'][$key][$index]);
        //     }

        //     // Đánh số lại các key
        //     $_FILES['attributes'][$key] = array_values($_FILES['attributes'][$key]);
        // }
        ...
        //     'attributes' => serialize($this->parse_attributes_with_image($attributes)),
        //     'post_type' => $post_type,
        //     'order' => $this->get_max_order($post_type) + 1,
        //     'status' => $this->input->post('status') ? 1 : 0,
        //     'created' => time(),
        //     'modified' => 0,
        //     'attributes_unserialize' => $attributes,
        //     'images_hidden' => $this->input->post('images'),
        //     'attributes_file' => $_FILES['attributes']
        // );

        $this->_data['data'] = isset($_FILES['attributes']) ? 'true' : 'false';

        $this->load->view('layout/site/partial/view_namtest', $this->_data);
    }
    ...
    function admin_update($id)
    {
        ...
        if ($post_type != $post_type_current) {
            $data['order'] = $this->get_max_order($post_type) + 1; //gia tri sap xep lon nhat cua menu mới
        }
    ...
    private function _upload_images($id, $input_name)
    {
        ...
        if (!empty($images) && isset($info['uploads'])) {
            $attributes = isset($row['attributes']) ? @unserialize($row['attributes']) : null;

            if (is_array($attributes) && !empty($attributes)) {
                foreach ($images as $key => $value) {
                    if (trim($value) != trim($attributes[$key]['image'])) {
                        @unlink(FCPATH . $this->_path . $value);
                    }
                }
            }
        }
    ...
    public function parse_attributes_with_image($attributes)
    {
        $data = array();
        $bool_label = isset($attributes['label']) && is_array($attributes['label']);
        $bool_content = isset($attributes['content']) && is_array($attributes['content']);
        $bool_image = isset($attributes['image']) && is_array($attributes['image']);
        if ($bool_label && $bool_content && $bool_image) {
            $label = $attributes['label'];
            $content = $attributes['content'];
