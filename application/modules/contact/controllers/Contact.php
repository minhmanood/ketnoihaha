...
class Contact extends Layout {
    ...
    function index() {
        ...
        $info_address = array();
        if(isset($this->_data['info_address_none'])){
            $row = $this->_data['info_address_none'];
            $attributes = isset($row['attributes']) ? @unserialize($row['attributes']) : null;
            $i = 0;
            $info_address['company'] = isset($attributes[$i]['content']) ? $attributes[$i]['content'] : '';
            $i++;
            $info_address['address'] = isset($attributes[$i]['content']) ? $attributes[$i]['content'] : '';
            $i++;
            $info_address['email'] = isset($attributes[$i]['content']) ? $attributes[$i]['content'] : '';
            $i++;
            $info_address['phone'] = isset($attributes[$i]['content']) ? $attributes[$i]['content'] : '';
        }
        $this->_data['info_address'] = $info_address;
        ...
        if (!empty($post)) {
            ...
            // $this->form_validation->set_rules('address', 'Địa chỉ tư vấn', 'trim|required');
            $this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|max_length[60]');
            $this->form_validation->set_rules('subject', 'Chủ đề', 'trim|required');
            ...
            if ($this->form_validation->run($this)) {
                ...
                if ($this->M_contact->add($data)) {
                    ...
                    $data_sendmail = array(
                        ...
                        'receiver_email' => 'lenhan10th@gmail.com',//$this->_data['email'], //mail nhan thư
                        'subject' => 'Liên hệ',
