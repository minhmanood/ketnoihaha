...
class Posts extends Layout {
    ...
    function get_items_cat_type($cat_type = '', $number = 8, $options = array()) {
    ...
    function site_details() {
        ...
        if (!empty($row)) {
            ...
            $image = $row['homeimgfile'];
            if (trim($title_seo) != '') {
                $this->_data['title_seo'] = $title_seo . ' - ' . $row['categories']['name'] . ' - ' . $this->_data['title_seo'];
            }
            if (trim($keywords) != '') {
            ...
            if (trim($image) != '') {
                $this->_data['image'] = get_media('posts', $image, 'no-image-thumb.png', '336x224x1');
            }
        } else {
            show_404();
        }
        ...
        // $partial['data'] = $row;
        // $this->load->view('layout/site/partial/view_namtest', $partial);
    ...
    function site_items_in_cat() {
        ...
        $row = modules::run('posts/postcat/get_in_alias', $cat_alias);
        ...
        if (!empty($row)) {
            ...
            $other_seo = $row['other_seo'];
            $h1_seo = $row['h1_seo'];
            $image = isset($row['image_fb']) ? $row['image_fb'] : '';
            if (trim($title_seo) != '') {
            ...
            if(trim($image) != ''){
                $this->_data['image'] = get_media('posts_cat', $image, 'no-image');
            }
        } else {
            show_404();
        }
        ...
        // Đổ dữ liệu vào view post_item
        $this->_data['post_list'] = $this->load->view('layout/site/partial/post_item', $partial, true);
        
        $partial = array();
        $rows = modules::run('posts/postcat/gets');
        $partial['list_postcats'] = $rows;
        if (!empty($rows)) {
            foreach ($rows as $key => $value) {
                $args['cat_alias'] = $value['alias'];
                $partial['list_postcats'][$key]['numbers_of_posts'] = $this->M_posts->counts($args);
            }
        }

        // Không lấy bài viết theo loại nữa
        unset($args['cat_alias']);
        $partial['list_posts_recently'] = $this->M_posts->gets($args, 5, 0);

        $partial['list_tags'] = modules::run('tags/gets', array(), 10, 0);

        $this->_data['post_sidebar'] = $this->load->view('layout/site/partial/post_sidebar', $partial, true);

        $this->_breadcrumbs[] = array(
            'url' => site_url($this->config->item('url_posts_cat') . '/' . $row['alias']),
            'name' => $row['name']
        );
        $this->set_breadcrumbs();
