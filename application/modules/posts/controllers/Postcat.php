...
class Postcat extends Layout {
    ...
    function __construct() {
        ...
        $this->_path = get_module_path('posts_cat');
    }
            ...
            if ($this->form_validation->run($this)) {
                if ($this->input->post('id')) {
                    ...
                    if ($this->admin_update($id)) {
                        $this->_upload_images_fb($id, 'image_fb');
                        $this->output->clearCache();
                        $notify_type = 'success';
                    ...
                    if ($insert_id != 0) {
                        $this->_upload_images_fb($insert_id, 'image_fb');
                        $this->output->clearCache();
                        $notify_type = 'success';
        ...
        if ($id != 0) {
            ...
            if (empty($has_child) && empty($posts_in_cat)) {
                if ($this->M_postcat->delete($id)) {
                    @unlink(FCPATH . $this->_path . $row['image_fb']);
    ...
    private function _upload_images_fb($id, $input_name) {
        $row = $this->get($id);
        ...
        if (isset($info['uploads'])) {
            ...
            if ($_FILES[$input_name]['size'] != 0) {
                foreach ($upload_images as $value) {
                    ...
                    $this->M_postcat->update($id, $data_images);
                }
