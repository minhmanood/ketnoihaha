<?php

class M_tags extends CI_Model {

    public $_table_name = 'tags';

    public function __construst() {
        parent::__construst();
    }

    private function generate_where($args) {
        if (isset($args['name'])) {
            $this->db->like($this->_table_name . ".name", $args['name']);
        }

        if (isset($args['in_id'])) {
            $this->db->where_in($this->_table_name . ".id", $args['in_id']);
        }
    }

    private function generate_order_by($args) {
        $allow_sort = array("DESC", "ASC");

        if (isset($args['order_by']) && is_array($args['order_by']) && !empty($args['order_by'])) {
            foreach ($args['order_by'] as $key => $value) {
                $sort = in_array($value, $allow_sort) ? $value : "DESC";
                $this->db->order_by($this->_table_name . '.' . $key, $sort);
            }
        }
    }

    public function gets($args, $perpage = 5, $offset = -1) {
        $this->db->distinct();
        $this->db->select('name');
        $this->db->from($this->_table_name);
        $this->db->join('tags_relationship', 'tags_relationship.tag_id = ' . $this->_table_name . '.id', 'left');
        $this->db->where('name != ', '');
        $this->db->where('tags_relationship.module', 'post');

        $this->generate_where($args);
        $this->generate_order_by($args);
        if ($offset >= 0) {
            $this->db->limit($perpage, $offset);
        }
        $query = $this->db->get();

        return $query->result_array();
    }

    public function counts($args) {
        $this->db->select('*');
        $this->db->from($this->_table_name);

        $this->generate_where($args);

        $query = $this->db->get();

        return $query->num_rows();
    }

    public function get($id) {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->where($this->_table_name . '.id', $id);
        $query = $this->db->get();
        return $query->row_array();
    }

    function get_data_by_alias($alias = '', $module = '') {
        $this->db->select();
        $this->db->from($this->_table_name);
        $this->db->join('tags_relationship', 'tags_relationship.tag_id = ' . $this->_table_name . '.id', 'left');
        $this->db->where($this->_table_name . '.alias', $alias);
        $this->db->where('tags_relationship.module', $module);
        $query = $this->db->get();

        return $query->result_array();
    }
    
    function get_data_by_name($name = '', $module = '') {
        $this->db->select();
        $this->db->from($this->_table_name);
        $this->db->join('tags_relationship', 'tags_relationship.tag_id = ' . $this->_table_name . '.id', 'left');
        $this->db->where('tags_relationship.module', $module);
        $this->db->like($this->_table_name . '.name', $name);
        $query = $this->db->get();

        return $query->result_array();
    }

    public function add($data = array()) {
        if (empty($data)) {
            return 0;
        }
        $query = $this->db->insert($this->_table_name, $data);
        $insert_id = $this->db->insert_id();
        return (isset($query)) ? $insert_id : 0;
    }

    public function update($id, $data) {
        if (empty($data)) {
            return FALSE;
        }
        $this->db->where('id', $id);
        $query = $this->db->update($this->_table_name, $data);

        return (isset($query)) ? true : false;
    }

    public function delete($in_id) {
        $this->db->where_in('id', $in_id);
        $query = $this->db->delete($this->_table_name);

        return (isset($query)) ? true : false;
    }

}

/* End of file m_tags.php */
/* Location: ./application/modules/tags/models/m_tags.php */